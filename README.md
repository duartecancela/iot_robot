...
```text
iot_robot/
├─ backend/       # Node.js backend (MQTT, REST API, Socket.IO, MongoDB logging)
├─ docs/          # Documentation, topics, db configuration
├─ esp32/         # ESP32 firmware (motors, sensors, MQTT telemetry)
├─ frontend/      # React web dashboard (telemetry, stream, control UI)
├─ rpi/           # Raspberry Pi services (camera stream, tracking, vision)
├─ shared/        # Shared configuration files
├─ simulator/     # Python MQTT robot simulator
├─ logs/          # Runtime logs generated by development scripts
├─ start_dev.sh   # Start development stack
├─ stop_dev.sh    # Stop development stack
├─ .gitignore
└─ README.md
```
```
